...
public partial class Depo_OtherthanTextbookBlockPradayDepo : System.Web.UI.Page
{
    DataSet ds;
    DataTable Dt, dt;
    ...
    private void FillLetterNo()
    {
      ...
      //  ddlLetterNo.SelectedValue = "807 A";
       // ddlLetterNo.Enabled = false;
    }
    private void FillReport()
    {
        Dt = new DataTable();
        //  objReports.DemandID = int.Parse(ddlLetterNo.SelectedValue);
        Dt = comm.FillDatasetByProc("call USP_OtherhanBlockWiseDepo('" + ddlLetterNo.SelectedValue + "','"+Session["UserID"]+"')").Tables[0];

        if (Dt.Rows.Count > 0)
        {
            ReportDataSource rds = new ReportDataSource("DataSet1", Dt);
            RVDistrictSupply.LocalReport.ReportPath = Server.MapPath("BRCOtherthantexbook.rdlc");
