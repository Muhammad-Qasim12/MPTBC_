...
public partial class Distrubution_StockBarCode : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            {
                int TotBooks = Convert.ToInt32(Request.QueryString["NoOfBooks"]);
                ...
                int D = 1, TotBarCode = 0, LastBarCodeNo = 0;
                float divi = 0;
                TotBarCode = (EndNo - StartNo) + 1;
                divi = (TotBooks / (TotBarCode * NoOfBooks));
                if (divi == 1 || divi == 1.0)
                {
                    LastBarCodeNo = TotBooks - (NoOfBooks * TotBarCode);
                }
                else
                {
                    TotBarCode = (EndNo - StartNo);
                    LastBarCodeNo = (TotBooks - (NoOfBooks * TotBarCode)) - 1;
                }
                ...
                for (int i = StartNo; i <= EndNo; i++)
                {
                    ...
                    if (i == EndNo)
                    {
                        // BarCodeNewNo = ((Bookstart + NoOfBooks - 1) - LastBarCodeNo).ToString();
                        if (LastBarCodeNo == 0)
                        {
                            BarCodeNewNo = (Bookstart + NoOfBooks - 1).ToString();
                        }
                        else
                        {
                            BarCodeNewNo = ((Bookstart) + LastBarCodeNo).ToString();
                        }
                    }
                    else
                    {
        ...
        <br><br><ul style='margin:0;padding:0;'> ";
                    ...
                    InnerHtmlTag = InnerHtmlTag + "<li style='float: left; width: 50%; text-align: center; list-style:none; height:300px;'><span style='margin:0!important;padding:0px!important'> प्रारंभिक स्टॉक </span><br> " +
                     "<span style='margin:0!important;padding:0px!important'> डिपो :-" + DepoName + " <br>" + title + " (" + BookType + ")</span><br>" +
                     "<span class='BarCode'>" + "*" + i.ToString() + "*" + "</span><br>" +
                      "<span style='margin-top:0px;padding:0px!important'> बंडल क्रमांक :" + i + " </span><br>" +
                    "<span style='margin-top:0px;padding:0px!important'> पुस्तक क्रमांक :" + Bookstart + " से " + BarCodeNewNo + " तक </span><br>" +
                     "<span style='margin-top:0px;padding:0px!important'> बंडल में कुल पुस्तके: " + NoOfBooks + " </span><br>" +
                   "<span style='margin-top:0px;padding:0px!important'> </span></li>  ";
