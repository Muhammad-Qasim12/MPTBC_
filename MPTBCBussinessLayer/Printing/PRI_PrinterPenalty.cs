...
{
    public class PRI_PrinterPenalty : ICommon
    {
        private Int32 _ID;
        ...
        {
            DBAccess obDBAccess = new DBAccess();
            obDBAccess.execute("USP_PRI008_PenaltyDetailLoad", DBAccess.SQLType.IS_PROC);
            obDBAccess.addParam("mPenaltyID_I", PenaltyID_I);
        ...
        public DataSet FillJobCodeDetail() 
        {
            ...
            try
            {
                obDbaccess.execute("USP_PRI009_JobCodeDetailLoadForPrinterPenalty", DBAccess.SQLType.IS_PROC);
        ...
        public DataSet TitleFill(int mGroupId)
        {
            DBAccess obDbaccess = new DBAccess();
            DataSet ds = new DataSet();
            try
            {
                obDbaccess.execute("USP_PRI009_PrinterPenaltyDtlShow", DBAccess.SQLType.IS_PROC);
                obDbaccess.addParam("mtype", 0);
                obDbaccess.addParam("mjobId", 0);
                obDbaccess.addParam("mGroupId", mGroupId);
        ...
        public DataSet PenaltyAmt(int mTitleId)
        {
            DBAccess obDbaccess = new DBAccess();
            DataSet ds = new DataSet();
            try
            {
                ...
                obDbaccess.addParam("mtype", 0);
                obDbaccess.addParam("mjobId", 0);
                obDbaccess.addParam("mGroupId", 0);
                obDbaccess.addParam("mTitleId", mTitleId);
                ds = obDbaccess.records();
            }
            catch (Exception ex) { }
            finally { obDbaccess = null; }
        ...
        public DataSet BookDistributionDtl(int PrinterID, string AcYear)
        {
            DBAccess obDbaccess = new DBAccess();
            DataSet ds = new DataSet();
            try
            {
                obDbaccess.execute("USP_PRI004_printerBookDistribution", DBAccess.SQLType.IS_PROC);
